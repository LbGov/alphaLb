...
(function (window, undefined) {
    var document = window.document, navigator = window.navigator, location = window.location; var jQuery = (function () {
        var jQuery = function (selector, context) { return new jQuery.fn.init(selector, context, rootjQuery); }, _jQuery = window.jQuery, _$ = window.$, rootjQuery, quickExpr = /^(?:[^#<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/, rnotwhite = /\S/, trimLeft = /^\s+/, trimRight = /\s+$/, rsingleTag = /^<(\w+)\s*\/?>(?:<\/\1>)?$/, rvalidchars = /^[\],:{}\s]*$/, rvalidescape = /\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, rvalidtokens = /"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g, rvalidbraces = /(?:^|:|,)(?:\s*\[)+/g, rwebkit = /(webkit)[ \/]([\w.]+)/, ropera = /(opera)(?:.*version)?[ \/]([\w.]+)/, rmsie = /(msie) ([\w.]+)/, rmozilla = /(mozilla)(?:.*? rv:([\w.]+))?/, rdashAlpha = /-([a-z]|[0-9])/ig, rmsPrefix = /^-ms-/, fcamelCase = function (all, letter) { return (letter + "").toUpperCase(); }, userAgent = navigator.userAgent, browserMatch, readyList, DOMContentLoaded, toString = Object.prototype.toString, hasOwn = Object.prototype.hasOwnProperty, push = Array.prototype.push, slice = Array.prototype.slice, trim = String.prototype.trim, indexOf = Array.prototype.indexOf, class2type = {}; jQuery.fn = jQuery.prototype = {
            ...
            }, selector: "", jquery: "1.7.2", length: 0, size: function () { return this.length; }, toArray: function () { return slice.call(this, 0); }, get: function (num) { return num == null ? this.toArray() : (num < 0 ? this[this.length + num] : this[num]); }, pushStack: function (elems, name, selector) {
            ...
            }, isFunction: function (obj) { return jQuery.type(obj) === "function"; }, isArray: Array.isArray || function (obj) { return jQuery.type(obj) === "array"; }, isWindow: function (obj) { return obj != null && obj == obj.window; }, isNumeric: function (obj) { return !isNaN(parseFloat(obj)) && isFinite(obj); }, type: function (obj) { return obj == null ? String(obj) : class2type[toString.call(obj)] || "object"; }, isPlainObject: function (obj) {
    ...
    jQuery.Callbacks = function (flags) {
        ...
        list = [], stack = [], memory, fired, firing, firingStart, firingLength, firingIndex, add = function (args) { var i, length, elem, type, actual; for (i = 0, length = args.length; i < length; i++) { elem = args[i]; type = jQuery.type(elem); if (type === "array") { add(elem); } else if (type === "function") { if (!flags.unique || !self.has(elem)) { list.push(elem); } } } }, fire = function (context, args) {
            args = args || []; memory = !flags.memory || [context, args]; fired = true; firing = true; firingIndex = firingStart || 0; firingStart = 0; firingLength = list.length; for (; list && firingIndex < firingLength; firingIndex++) { if (list[firingIndex].apply(context, args) === false && flags.stopOnFalse) { memory = true; break; } }
    ...
    sliceDeferred = [].slice; jQuery.extend({
        ...
        fragment.removeChild(div); fragment = select = opt = div = input = null; jQuery(function () {
            var container, outer, inner, table, td, offsetSupport, marginDiv, conMarginTop, style, html, positionTopLeftWidthHeight, paddingMarginBorderVisibility, paddingMarginBorder, body = document.getElementsByTagName("body")[0]; if (!body) { return; }
            ...
            body.removeChild(container); marginDiv = div = container = null; jQuery.extend(support, offsetSupport);
        }); return support;
            ...
            var parts, part, attr, name, l, elem = this[0], i = 0, data = null; if (key === undefined) {
                if (this.length) {
                    data = jQuery.data(elem); if (elem.nodeType === 1 && !jQuery._data(elem, "parsedAttrs")) {
                        attr = elem.attributes; for (l = attr.length; i < l; i++) { name = attr[i].name; if (name.indexOf("data-") === 0) { name = jQuery.camelCase(name.substring(5)); dataAttr(elem, name, data[name]); } }
        ...
        if (data === undefined && elem.nodeType === 1) {
            var name = "data-" + key.replace(rmultiDash, "-$1").toLowerCase(); data = elem.getAttribute(name); if (typeof data === "string") {
                try { data = data === "true" ? true : data === "false" ? false : data === "null" ? null : jQuery.isNumeric(data) ? +data : rbrace.test(data) ? jQuery.parseJSON(data) : data; } catch (e) { }
    ...
    jQuery.extend({
        ...
        queue: function (type, data) {
            ...
            return data === undefined ? this : this.each(function () { var queue = jQuery.queue(this, type, data); if (type === "fx" && queue[0] !== "inprogress") { jQuery.dequeue(this, type); } });
            ...
            resolve(); return defer.promise(object);
        }
        ...
        valHooks: {
            ...
            var propName, attrNames, name, l, isBool, i = 0; if (value && elem.nodeType === 1) {
                attrNames = value.toLowerCase().split(rspace); l = attrNames.length; for (; i < l; i++) {
        ...
        }; jQuery.attrHooks.tabindex.set = nodeHook.set; jQuery.each(["width", "height"], function (i, name) { jQuery.attrHooks[name] = jQuery.extend(jQuery.attrHooks[name], { set: function (elem, value) { if (value === "") { elem.setAttribute(name, "auto"); return value; } } }); }); jQuery.attrHooks.contenteditable = {
    ...
    jQuery.Event.prototype = {
        ...
        jQuery.event.special.submit = {
            setup: function () {
                ...
                jQuery.event.add(this, "click._submit keypress._submit", function (e) { var elem = e.target, form = jQuery.nodeName(elem, "input") || jQuery.nodeName(elem, "button") ? elem.form : undefined; if (form && !form._submit_attached) { jQuery.event.add(form, "submit._submit", function (event) { event._submit_bubble = true; }); form._submit_attached = true; } });
    ...
    jQuery.fn.extend({
        on: function (types, selector, data, fn, one) {
            ...
            if (types && types.preventDefault && types.handleObj) { var handleObj = types.handleObj; jQuery(types.delegateTarget).off(handleObj.namespace ? handleObj.origType + "." + handleObj.namespace : handleObj.origType, handleObj.selector, handleObj.handler); return this; }
        ...
        Expr.match.globalPOS = origPOS; var makeArray = function (array, results) {
    ...
    })(); var runtil = /Until$/, rparentsprev = /^(?:parents|prevUntil|prevAll)/, rmultiselector = /,/, isSimple = /^.[^:#\[\.,]*$/, slice = Array.prototype.slice, POS = jQuery.expr.match.globalPOS, guaranteedUnique = { children: true, contents: true, next: true, prev: true }; jQuery.fn.extend({
    ...
    jQuery.fn.extend({
        text: function (value) { return jQuery.access(this, function (value) { return value === undefined ? jQuery.text(this) : this.empty().append((this[0] && this[0].ownerDocument || document).createTextNode(value)); }, null, value, arguments.length); }, wrapAll: function (html) {
            ...
            return jQuery.access(this, function (value) {
                var elem = this[0] || {}, i = 0, l = this.length; if (value === undefined) { return elem.nodeType === 1 ? elem.innerHTML.replace(rinlinejQuery, "") : null; }
            ...
            if (this[0]) {
                ...
                if (scripts.length) {
                    jQuery.each(scripts, function (i, elem) {
                        if (elem.src) { jQuery.ajax({ type: "GET", global: false, url: elem.src, async: false, dataType: "script" }); } else { jQuery.globalEval((elem.text || elem.textContent || elem.innerHTML || "").replace(rcleanScript, "/*$0*/")); }
                        if (elem.parentNode) { elem.parentNode.removeChild(elem); }
                    });
                }
            }
    ...
    jQuery.extend({
        clone: function (elem, dataAndEvents, deepDataAndEvents) {
            ...
            for (var i = 0, elem; (elem = elems[i]) != null; i++) {
                ...
                if (typeof elem === "string") {
                    if (!rhtml.test(elem)) { elem = context.createTextNode(elem); } else {
                        elem = elem.replace(rxhtmlTag, "<$1></$2>"); var tag = (rtagName.exec(elem) || ["", ""])[1].toLowerCase(), wrap = wrapMap[tag] || wrapMap._default, depth = wrap[0], div = context.createElement("div"), safeChildNodes = safeFragment.childNodes, remove; if (context === document) { safeFragment.appendChild(div); } else { createSafeFragment(context).appendChild(div); }
            ...
            if (fragment) {
                checkScriptType = function (elem) { return !elem.type || rscriptType.test(elem.type); }; for (i = 0; ret[i]; i++) {
                    script = ret[i]; if (scripts && jQuery.nodeName(script, "script") && (!script.type || rscriptType.test(script.type))) { scripts.push(script.parentNode ? script.parentNode.removeChild(script) : script); } else {
                        if (script.nodeType === 1) { var jsTags = jQuery.grep(script.getElementsByTagName("script"), checkScriptType); ret.splice.apply(ret, [i + 1, 0].concat(jsTags)); }
    ...
    }); var ralpha = /alpha\([^)]*\)/i, ropacity = /opacity=([^)]*)/, rupper = /([A-Z]|^ms)/g, rnum = /^[\-+]?(?:\d*\.)?\d+$/i, rnumnonpx = /^-?(?:\d*\.)?\d+(?!px)[^\d\s]+$/i, rrelNum = /^([\-+])=([\-+.\de]+)/, rmargin = /^margin/, cssShow = { position: "absolute", visibility: "hidden", display: "block" }, cssExpand = ["Top", "Right", "Bottom", "Left"], curCSS, getComputedStyle, currentStyle; jQuery.fn.css = function (name, value) { return jQuery.access(this, function (elem, name, value) { return value !== undefined ? jQuery.style(elem, name, value) : jQuery.css(elem, name); }, name, value, arguments.length > 1); }; jQuery.extend({
        cssHooks: { opacity: { get: function (elem, computed) { if (computed) { var ret = curCSS(elem, "opacity"); return ret === "" ? "1" : ret; } else { return elem.style.opacity; } } } }, cssNumber: { "fillOpacity": true, "fontWeight": true, "lineHeight": true, "opacity": true, "orphans": true, "widows": true, "zIndex": true, "zoom": true }, cssProps: { "float": jQuery.support.cssFloat ? "cssFloat" : "styleFloat" }, style: function (elem, name, value, extra) {
            ...
            var old = {}, ret, name; for (name in options) { old[name] = elem.style[name]; elem.style[name] = options[name]; }
            ret = callback.call(elem); for (name in options) { elem.style[name] = old[name]; }
            return ret;
        }
    }); jQuery.curCSS = jQuery.css; if (document.defaultView && document.defaultView.getComputedStyle) {
        getComputedStyle = function (elem, name) {
            var ret, defaultView, computedStyle, width, style = elem.style; name = name.replace(rupper, "-$1").toLowerCase(); if ((defaultView = elem.ownerDocument.defaultView) && (computedStyle = defaultView.getComputedStyle(elem, null))) { ret = computedStyle.getPropertyValue(name); if (ret === "" && !jQuery.contains(elem.ownerDocument.documentElement, elem)) { ret = jQuery.style(elem, name); } }
    ...
    curCSS = getComputedStyle || currentStyle; function getWidthOrHeight(elem, name, extra) {
        var val = name === "width" ? elem.offsetWidth : elem.offsetHeight, i = name === "width" ? 1 : 0, len = 4; if (val > 0) {
            if (extra !== "border") {
                for (; i < len; i += 2) {
                    if (!extra) { val -= parseFloat(jQuery.css(elem, "padding" + cssExpand[i])) || 0; }
        ...
        val = parseFloat(val) || 0; if (extra) {
            for (; i < len; i += 2) {
                val += parseFloat(jQuery.css(elem, "padding" + cssExpand[i])) || 0; if (extra !== "padding") { val += parseFloat(jQuery.css(elem, "border" + cssExpand[i] + "Width")) || 0; }
                if (extra === "margin") { val += parseFloat(jQuery.css(elem, extra + cssExpand[i])) || 0; }
            }
        }
        return val + "px";
    }
    jQuery.each(["height", "width"], function (i, name) { jQuery.cssHooks[name] = { get: function (elem, computed, extra) { if (computed) { if (elem.offsetWidth !== 0) { return getWidthOrHeight(elem, name, extra); } else { return jQuery.swap(elem, cssShow, function () { return getWidthOrHeight(elem, name, extra); }); } } }, set: function (elem, value) { return rnum.test(value) ? value + "px" : value; } }; }); if (!jQuery.support.opacity) {
        ...
        load: function (url, params, callback) {
            ...
            var self = this; jQuery.ajax({
                url: url, type: type, dataType: "html", data: params, complete: function (jqXHR, status, responseText) {
                    responseText = jqXHR.responseText; if (jqXHR.isResolved()) { jqXHR.done(function (r) { responseText = r; }); self.html(selector ? jQuery("<div>").append(responseText.replace(rscript, "")).find(selector) : responseText); }
    ...
    }); function buildParams(prefix, obj, traditional, add) { if (jQuery.isArray(obj)) { jQuery.each(obj, function (i, v) { if (traditional || rbracket.test(prefix)) { add(prefix, v); } else { buildParams(prefix + "[" + (typeof v === "object" ? i : "") + "]", v, traditional, add); } }); } else if (!traditional && jQuery.type(obj) === "object") { for (var name in obj) { buildParams(prefix + "[" + name + "]", obj[name], traditional, add); } } else { add(prefix, obj); } }
    ...
    var jsc = jQuery.now(), jsre = /(\=)\?(&|$)|\?\?/i; jQuery.ajaxSetup({ jsonp: "callback", jsonpCallback: function () { return jQuery.expando + "_" + (jsc++); } }); jQuery.ajaxPrefilter("json jsonp", function (s, originalSettings, jqXHR) {
        var inspectData = (typeof s.data === "string") && /^application\/x\-www\-form\-urlencoded/.test(s.contentType); if (s.dataTypes[0] === "jsonp" || s.jsonp !== false && (jsre.test(s.url) || inspectData && jsre.test(s.data))) {
    ...
    var elemdisplay = {}, iframe, iframeDoc, rfxtypes = /^(?:toggle|show|hide)$/, rfxnum = /^([+\-]=)?([\d+.\-]+)([a-z%]*)$/i, timerId, fxAttrs = [["height", "marginTop", "marginBottom", "paddingTop", "paddingBottom"], ["width", "marginLeft", "marginRight", "paddingLeft", "paddingRight"], ["opacity"]], fxNow; jQuery.fn.extend({
        show: function (speed, easing, callback) {
            ...
            prop = jQuery.extend({}, prop); function doAnimation() {
                ...
                var opt = jQuery.extend({}, optall), isElement = this.nodeType === 1, hidden = isElement && jQuery(this).is(":hidden"), name, val, p, e, hooks, replace, parts, start, end, unit, method; opt.animatedProperties = {}; for (p in prop) {
    ...
    jQuery.each({ slideDown: genFx("show", 1), slideUp: genFx("hide", 1), slideToggle: genFx("toggle", 1), fadeIn: { opacity: "show" }, fadeOut: { opacity: "hide" }, fadeToggle: { opacity: "toggle" } }, function (name, props) { jQuery.fn[name] = function (speed, easing, callback) { return this.animate(props, speed, easing, callback); }; }); jQuery.extend({
        ...
        }, easing: { linear: function (p) { return p; }, swing: function (p) { return (-Math.cos(p * Math.PI) / 2) + 0.5; } }, timers: [], fx: function (elem, options, prop) { this.options = options; this.elem = elem; this.prop = prop; options.orig = options.orig || {}; }
    }); jQuery.fx.prototype = {
        update: function () {
    ...
    var getOffset, rtable = /^t(?:able|d|h)$/i, rroot = /^(?:body|html)$/i; if ("getBoundingClientRect" in document.documentElement) {
        ...
        getOffset = function (elem, doc, docElem) {
            var computedStyle, offsetParent = elem.offsetParent, prevOffsetParent = elem, body = doc.body, defaultView = doc.defaultView, prevComputedStyle = defaultView ? defaultView.getComputedStyle(elem, null) : elem.currentStyle, top = elem.offsetTop, left = elem.offsetLeft; while ((elem = elem.parentNode) && elem !== body && elem !== docElem) {
    ...
    jQuery.fn.offset = function (options) {
        ...
        var top = /Y/.test(prop); jQuery.fn[method] = function (val) {
            return jQuery.access(this, function (elem, method, val) {
                var win = getWindow(elem); if (val === undefined) { return win ? (prop in win) ? win[prop] : jQuery.support.boxModel && win.document.documentElement[method] || win.document.body[method] : elem[method]; }
    ...
    jQuery.each({ Height: "height", Width: "width" }, function (name, type) {
        var clientProp = "client" + name, scrollProp = "scroll" + name, offsetProp = "offset" + name; jQuery.fn["inner" + name] = function () { var elem = this[0]; return elem ? elem.style ? parseFloat(jQuery.css(elem, type, "padding")) : this[type]() : null; }; jQuery.fn["outer" + name] = function (margin) { var elem = this[0]; return elem ? elem.style ? parseFloat(jQuery.css(elem, type, margin ? "margin" : "border")) : this[type]() : null; }; jQuery.fn[type] = function (value) {
            return jQuery.access(this, function (elem, type, value) {
                var doc, docElemProp, orig, ret; if (jQuery.isWindow(elem)) { doc = elem.document; docElemProp = doc.documentElement[clientProp]; return jQuery.support.boxModel && docElemProp || doc.body && doc.body[clientProp] || docElemProp; }
                if (elem.nodeType === 9) {
                    doc = elem.documentElement; if (doc[clientProp] >= doc[scrollProp]) { return doc[clientProp]; }
...
/****** FILE: mysite/javascript/govtnz-theme.js *****/

(function ($) {
    $(document).ready(function () {
        $('a').filter(function () { if ($(this).attr('target') === '_blank') { return true; } }).each(function () { $(this).attr('rel', 'noopener noreferrer'); }); function searchPlaceholder() { if (document.createElement("input").placeholder === undefined) { var value = $("input#searchterm").attr("placeholder"); $("input#searchterm").css('color', '#777'); $("input#searchterm").val(value); $("input#searchterm").focus(function () { if ($("input#searchterm").val() === value) { $("input#searchterm").val(""); $("input#searchterm").css('color', '#000'); } }); $("input#searchterm").blur(function () { if ($("input#searchterm").val() === "") { $("input#searchterm").val(value); $("input#searchterm").css('color', '#777'); } }); } }
        setTimeout(searchPlaceholder, 500); function markExternalLinks() { $("a:not(a:has(img))").each(function () { if (this.protocol.indexOf('http') === 0 && this.host.length && this.host.indexOf(location.host) !== 0) { if (!$(this).hasClass('no-external-link')) { $(this).attr('rel', 'external'); $(this).addClass('external'); $(this).append("&nbsp;<i class='fa fa-external-link'></i>"); $(this).append("<span class='external-text' title='External site link'>(external site link)</span>"); } } }); }
        function setUrgentAlert() { var banner = $('#urgent-banner'); var alertText = $(banner).attr('data-alert-text'); var target = $(banner).find('.container'); $(target).find('.text').html(alertText); $("#urgent-banner a:not(a:has(img))").each(function () { if (this.protocol.indexOf('http') === 0 && this.host.length && this.host.indexOf(location.host) !== 0) { if (!$(this).hasClass('no-external-link')) { $(this).attr('rel', 'external'); $(this).addClass('external'); $(this).append("&nbsp;<i class='fa fa-external-link'></i>"); $(this).append("<span class='external-text' title='External site link'>(external site link)</span>"); } } }); }
        setTimeout(setUrgentAlert, 500); markExternalLinks(); $("a[href^='/']").addClass('internal-link'); $("a[href^='browse/']").addClass('internal-link-slash'); if ($(".pagetype-process")[0]) {
            var $main = $('.main-content'); var showStepNumbers = $main.hasClass('show-step-numbers'); if (!location.hash) { var target = getFirstStep(); highlightSelection(target, 'external'); replaceURL(target); calculateNextPrev(); } else { var target = location.hash.replace('#', ''); highlightSelection(target, 'external'); replaceURL(target); calculateNextPrev(); }
            var i = 1; $('.process-step').each(function () {
                $(this).removeClass('css-numbered'); if (showStepNumbers) { $(this).find('.step-title').prepend('<span class="number">' + i + '.</span>'); }
                i++; var origID = $(this).attr('id'); $(this).attr('id', origID + '_hash');
            }); var i = 0; $(document).on("click", "summary, .accordion-expand", function (e) { var target = location.hash.replace('#', ''); highlightSelection(target, 'in-page'); }); $(document).on("click", ".process-nav a", function (e) { $(this).closest(".accordion-content").animate({ height: "toggle" }, 300, function () { var details = $(this).closest('details'); details.removeClass('accordion-heading-open'); details.removeAttr('open'); details.find('summary').attr('aria-expanded', 'false'); details.find('summary').removeAttr('style'); details.find('.accordion-open').show(); details.find('.accordion-close').hide(); }); var parent = $(this).closest('.title-row'); var target = $(this).attr('data-controls'), stepNumber = $(this).attr('data-step-number'); highlightSelection(target, 'in-page'); replaceURL(target); calculateNextPrev(); $('.step-title').focus(); e.preventDefault(); }); $(document).on("click", "#link-next", function (e) { var target = $(this).attr('href').split('#')[1]; highlightSelection(target, 'in-page'); replaceURL(target); setTimeout(function () { calculateNextPrev(); }, 1); $('.step-title').focus(); e.preventDefault(); }); $(document).on("click", "#link-prev", function (e) { var target = $(this).attr('href').split('#')[1]; highlightSelection(target, 'in-page'); replaceURL(target); setTimeout(function () { calculateNextPrev(); }, 1); $('.step-title').focus(); e.preventDefault(); }); $(window).on('popstate', function (e) {
                $('.process-nav.desktop .internal-link.selected').removeClass('selected'); var state = e.originalEvent.state; var target; if (state && state.stepId) { target = state.stepId; } else { target = getFirstStep(); }
                highlightSelection(target, 'in-page'); calculateNextPrev();
            });
        }
        function getFirstStep() { var firstStepHref = $(".process-nav.desktop a").first().attr('href'); var target = firstStepHref.split('#')[1]; return target; }
        function calculateNextPrev() {
            var selected = $(".process-nav.desktop a.selected"); var anchors = $(".process-nav.desktop a"); var pos = anchors.index(selected); if ((pos + 1) < anchors.length) { var next = anchors.get(pos + 1); $('#caption-next .title').html($(next).html()); var target = $(next).attr('data-controls'); $('#link-next').attr('href', window.location.href.split('#')[0] + '#' + target); $('.next-page-link').removeClass('hide'); $('.next-page-link').show(); } else { $('.next-page-link').hide(); $('#caption-next .title').html(''); $('#link-next').attr('href', ''); }
            if ((pos - 1) >= 0) { var prev = anchors.get(pos - 1); $('#caption-prev .title').html($(prev).html()); var target = $(prev).attr('data-controls'); $('#link-prev').attr('href', window.location.href.split('#')[0] + '#' + target); $('.prev-page-link').removeClass('hide'); $('.prev-page-link').show(); } else { $('.prev-page-link').hide(); $('#caption-next .prev').html(''); $('#link-prev').attr('href', ''); }
        }
        function highlightSelection(target, submissiontype) {
            $('.process-nav a').each(function () { $(this).closest('.process-nav li').removeClass('selected'); $(this).attr('tabindex', ''); }); $('.process-nav li a').each(function () { var processstep = $(this).attr('data-controls'); if (target == processstep) { $(this).addClass('selected'); $(this).attr('tabindex', -1); } }); $('.process-step').each(function () {
                if (submissiontype == 'in-page') { var step = target + '_hash'; } else { var step = target; }
                if (this.id == step) { $(this).show(); } else { $(this).hide(); }
            });
        }
        function replaceURL(target) {
            if (history.pushState) { history.pushState({ stepId: target }, null, window.location.pathname + '#' + target); }
            window.scrollTo(0, 0);
        }
        $('.step-button-jump,.step-button-next,.step-button-prev').on("click", function (event) { $('#next-focus-target').focus(); }); $(document).on("click", "#skip-nav", function (e) { $(".main-content").first().focus(); e.preventDefault(); }); $('.userform .field').each(function (index, element) {
            var
            oSpan = $(element).find('.right-title'), sContent = $(oSpan).html(); if (sContent !== null) {
                var
                iPos = $(element).attr('id').lastIndexOf('_'), oField = $(element).find('input, select, textarea').first(), oLabel = $(element).find('legend, label').first(), sID = 'description-' + $(element).attr('id').substring(iPos + 1); if (oLabel) { $('<div class="inline-details" id="' + sID + '">' + sContent + '</div>').insertAfter(oLabel); $(oSpan).remove(); if (oField) { $(oField).attr('aria-describedby', sID); } }
            }
        })
        $('form ul.optionset input:checked').closest('li').addClass('bordered'); $('form ul.optionset input').on('change', function (event) { toggleCheckRadioStatus('input', $(this)); }); $('form ul.optionset li').unbind('click'); $('form ul.optionset li').on('click', function (event) { var input = $(this).find('input'); toggleCheckRadioStatus('li', input); if ($(event.target).is('li') || $(event.target).is('label')) { event.preventDefault(); input.prop('checked', !input.prop('checked')); input.trigger('change'); } }); $('form ul.optionset input').on('focus', function (event) { $(this).closest('li').focus(); });
    }); function toggleCheckRadioStatus(sSource, oObject) { if (typeof oObject !== 'undefined' && oObject.data('change') !== sSource) { oObject.data('change', sSource); oObject.closest('ul').find('li').removeClass('bordered'); setTimeout(function (oObj) { oObj.data('change', ''); }, 200, oObject); switch (oObject.attr('type')) { case 'checkbox': oObject.closest('ul').find('li input:checked').closest('li').addClass('bordered'); break; case 'radio': oObject.closest('li').addClass('bordered'); if (oObject.prop('checked') !== 'true') { oObject.prop('checked', 'true'); oObject.focus(); } } } }
})(jQuery);;

/****** FILE: mysite/javascript/accessibility_polyfill.js *****/

$(document).ready(function () {
    if ($('html').hasClass('ie8')) { $('summary').each(function () { $(this).wrapInner('<h2></h2>'); }); return; }
    function supports_details() { return ('open' in document.createElement('details')); }
    $('.accordion-expand').each(function () { $(this).attr('aria-controls', ''); }); $('details').each(function (i) {
        var $details = $(this); var $summary = $('summary', $details).first(); var detailsWrap = $details.closest('.accordion-wrap'); var expandAll = detailsWrap.find('.accordion-toggle .accordion-expand'); $details.attr('role', 'group'); if (!$details.attr('id')) { $details.attr('id', 'details-' + i); expandAll.attr('aria-controls', expandAll.attr('aria-controls') + ' details-' + i); }
        var isOpen = $details.prop('open'); if (typeof isOpen === 'undefined') { isOpen = false; }
        $summary.attr({ 'role': 'button', 'aria-controls': $details.attr('id'), 'aria-expanded': isOpen }).click(function () { if ($details.attr('open')) { $summary.attr('aria-expanded', false); } else { $summary.attr('aria-expanded', true); } }); if (!supports_details()) { $details.addClass('no-details'); $summary.attr('tabindex', 0).keypress(function (event) { if (event.which == 13 || event.which == 32) { $summary.click(); } }).click(function () { if ($details.attr('open')) { $details.removeAttr('open'); } else { $details.attr('open', 'open'); } }); }
    });
});;

/****** FILE: rules-reduction/javascript/rules-reduction.js *****/

(function ($) {
    "use strict"; $(document).ready(function () {
        $('.SelectionGroup input.selector:not(:checked)').each(function () { $(this).siblings('.selectiongroup_item').hide(); }); $('.SelectionGroup input.selector').on('click', function () {
            var checked, inputType, container, relatedTextField; checked = $(this).prop('checked'); inputType = $(this).prop('type'); container = $(this).parents('.SelectionGroup'); relatedTextField = $(this).siblings('.selectiongroup_item'); if (inputType == 'radio') { container.find('.selectiongroup_item').hide(); container.find('.selectiongroup_item input').val(''); }
            if (checked) { relatedTextField.show(); } else { relatedTextField.hide(); relatedTextField.find('input').val(''); }
        }); $('#RulesReductionForm_RulesReductionForm').on('submit', function () { $(this).find('input.action').attr('disabled', 'disabled'); });
    });
})(jQuery);;

/****** FILE: themes/dia-theme/javascript/inline-feedback.js *****/

(function ($) { "use strict"; $(document).ready(function () { $('details').each(function (index) { var details = $(this); var summary = details.find('summary'); details.on('submit', '#Form_InlineFeedbackForm', function (event) { var inlineFeedbackForm = $(this); event.preventDefault(); $.ajax({ type: 'POST', url: inlineFeedbackForm.attr('action'), data: inlineFeedbackForm.serialize() }).done(function (data) { inlineFeedbackForm.replaceWith(data); $('.message').attr('tabindex', '-1').focus(); var $c = $(".inline-feedback-container"); if ($c.length > 0) { $c[0].scrollIntoView(true); var cont = $c[0]; if ($(cont).length) { $(cont).find("h2").focus(); } } }); }); }); $('details.open-by-default').each(function (i, el) { var $el = $(el); $('summary', $el).click(); }); }); })(jQuery);;

/****** FILE: themes/dia-theme/javascript/ie8fix.js *****/

var div = document.createElement("div"); div.innerHTML = "<!--[if lt IE 9]><i></i><![endif]-->"; if (isIeLessThan9 = (div.getElementsByTagName("i").length == 1)) {
    var src = ['themes/dia-theme/javascript/html5shiv.min.js', 'themes/dia-theme/javascript/respond.min.js']; for (var ipos = 0; ipos < src.length; ipos++) { var head = document.getElementsByTagName('head')[0]; var script = document.createElement('script'); script.type = 'text/javascript'; script.src = src[ipos]; head.appendChild(script); }
    var nav = document.createElement('nav');
};

/****** FILE: mysite/javascript/polyfills.js *****/

if (typeof String.prototype.trim !== 'function') { String.prototype.trim = function () { return this.replace(/^\s+|\s+$/g, ''); } }
Math.trunc = Math.trunc || function (x) { return x < 0 ? Math.ceil(x) : Math.floor(x); };

/****** FILE: themes/dia-theme/javascript/searchbar.js *****/
...
$(document).ready(function () {
    ...
    $(document).on("blur", "#searchterm", function (e) {
        if ($('.header-search-form #search-form #searchterm').val()) { $('.header-search-form #search-form .search-label').addClass('entered'); } else { $('.header-search-form #search-form .search-label').removeClass('entered'); }
        e.preventDefault();
    }); $(document).on("blur", "#searchterm2", function (e) {
        if ($('#search-form2 #searchterm2').val()) { $('#search-form2 .search-label').addClass('entered'); } else { $('#search-form2 .search-label').removeClass('entered'); $('#search-form2 .search-label').text('Search Govt.nz'); }
        e.preventDefault();
    });
});;

/****** FILE: mysite/javascript/jquery.simulate.js *****/
; (function ($, undefined) {
    var rkeyEvent = /^key/, rmouseEvent = /^(?:mouse|contextmenu)|click/; $.fn.simulate = function (type, options) { return this.each(function () { new $.simulate(this, type, options); }); }; $.simulate = function (elem, type, options) { var method = $.camelCase("simulate-" + type); this.target = elem; this.options = options; if (this[method]) { this[method](); } else { this.simulateEvent(elem, type, options); } }; $.extend($.simulate, { keyCode: { BACKSPACE: 8, COMMA: 188, DELETE: 46, DOWN: 40, END: 35, ENTER: 13, ESCAPE: 27, HOME: 36, LEFT: 37, NUMPAD_ADD: 107, NUMPAD_DECIMAL: 110, NUMPAD_DIVIDE: 111, NUMPAD_ENTER: 108, NUMPAD_MULTIPLY: 106, NUMPAD_SUBTRACT: 109, PAGE_DOWN: 34, PAGE_UP: 33, PERIOD: 190, RIGHT: 39, SPACE: 32, TAB: 9, UP: 38 }, buttonCode: { LEFT: 0, MIDDLE: 1, RIGHT: 2 } }); $.extend($.simulate.prototype, {
        simulateEvent: function (elem, type, options) { var event = this.createEvent(type, options); this.dispatchEvent(elem, type, event, options); }, createEvent: function (type, options) {
            if (rkeyEvent.test(type)) { return this.keyEvent(type, options); }
            if (rmouseEvent.test(type)) { return this.mouseEvent(type, options); }
        }, mouseEvent: function (type, options) {
            var event, eventDoc, doc, body; options = $.extend({ bubbles: true, cancelable: (type !== "mousemove"), view: window, detail: 0, screenX: 0, screenY: 0, clientX: 1, clientY: 1, ctrlKey: false, altKey: false, shiftKey: false, metaKey: false, button: 0, relatedTarget: undefined }, options); if (document.createEvent) {
                event = document.createEvent("MouseEvents"); event.initMouseEvent(type, options.bubbles, options.cancelable, options.view, options.detail, options.screenX, options.screenY, options.clientX, options.clientY, options.ctrlKey, options.altKey, options.shiftKey, options.metaKey, options.button, options.relatedTarget || document.body.parentNode); if (event.pageX === 0 && event.pageY === 0 && Object.defineProperty) {
                    eventDoc = event.relatedTarget.ownerDocument || document; doc = eventDoc.documentElement; body = eventDoc.body; Object.defineProperty(event, "pageX", {
                        get: function () {
            ...
            return event;
        }, dispatchEvent: function (elem, type, event) { if (elem.dispatchEvent) { elem.dispatchEvent(event); } else if (elem.fireEvent) { elem.fireEvent("on" + type, event); } }, simulateFocus: function () {
            var focusinEvent, triggered = false, element = $(this.target); function trigger() { triggered = true; }
            ...
            var focusoutEvent, triggered = false, element = $(this.target); function trigger() { triggered = true; }
            element.bind("blur", trigger); element[0].blur(); setTimeout(function () {
                if (element[0].ownerDocument.activeElement === element[0]) { element[0].ownerDocument.body.focus(); }
    ...
    }); function findCenter(elem) { var offset, document = $(elem.ownerDocument); elem = $(elem); offset = elem.offset(); return { x: offset.left + elem.outerWidth() / 2 - document.scrollLeft(), y: offset.top + elem.outerHeight() / 2 - document.scrollTop() }; }
    $.extend($.simulate.prototype, {
        simulateDrag: function () {
            var i = 0, target = this.target, options = this.options, center = findCenter(target), x = Math.floor(center.x), y = Math.floor(center.y), dx = options.dx || 0, dy = options.dy || 0, moves = options.moves || 3, coord = { clientX: x, clientY: y }; this.simulateEvent(target, "mousedown", coord); for (; i < moves; i++) { x += dx / moves; y += dy / moves; coord = { clientX: Math.round(x), clientY: Math.round(y) }; this.simulateEvent(document, "mousemove", coord); }
            this.simulateEvent(target, "mouseup", coord); this.simulateEvent(target, "click", coord);
        }
    });
})(jQuery);;

/****** FILE: themes/dia-theme/javascript/accordion.js *****/
...
(function ($) {
    ...
    if (this.find("Expand all")) {
        collapseTag = "Collapse all";
        expandTag = "Expand all";
    }
    else {
        collapseTag = "ضم الكل";
        expandTag = "توسيع الكل";
    }
    var animationDuration = 100; $('.accordion-list details summary').each(function () { $(this).nextAll().css('display', 'none'); }); $('.accordion-list details summary').click(function (e) { e.preventDefault(); var toggleState = $(this).parent('details').prop('open'); if (!toggleState) { $(this).parent('details').prop('open', true); } }); function accordionFire(oThis) {
        if ($(oThis).find(".accordion-expand").length) { $(oThis).data("canfire", "false"); var bCollapse = ($(oThis).find(".accordion-expand").html() === "Collapse all"); $(".accordion-item").each(function (index) { if (!$(this).hasClass('flag-contextual-accordion-item')) { accordionItemToggle(this, bCollapse); } }); }
        else
            accordionItemToggle($(oThis).parent());
    }
    function accordionItemToggle(oItem, bCollapse) {
        $(oItem).data("canfire", 'false'); var oContent = $(oItem).find(".accordion-content"); if (oContent.length) {
            if (typeof bCollapse === "undefined" || (bCollapse && oContent.is(":visible")) || (!bCollapse && !oContent.is(":visible"))) {
                oContent.animate({ height: "toggle" }, animationDuration, function () {
                    if (oContent.is(":visible")) { $(oItem).find("details").prop("open", true); $(oItem).find(".accordion-close").show(); $(oItem).find(".accordion-open").hide(); $(oItem).find("summary").css("color", "#121212"); $(oItem).addClass("accordion-heading-open"); $(".book-mobile-menu").addClass("accordion-heading-open"); $(oItem).find(".accordion-item-button").attr("aria-expanded", "true"); calculateExpanderState(oItem); } else { $(oContent).closest('details').removeProp("open"); $(oItem).find(".accordion-close").hide(); $(oItem).find(".accordion-open").show(); $(oItem).find("summary").css("color", "#336699"); $(oItem).removeClass("accordion-heading-open"); $(".book-mobile-menu").removeClass("accordion-heading-open"); $(oItem).find(".accordion-item-button").attr("aria-expanded", "false"); calculateExpanderState(oItem); }
    ...
    function calculateExpanderState(oItem) {
        oItem = $(oItem); var $expander, $items; if (oItem.is('button')) { oItem = null; }
        else if (oItem.hasClass('inline-accordion')) {
            $prior = oItem.prev(); while ($prior.is('details')) { $prior = $prior.prev(); }
            if ($prior.hasClass('accordion-toggle')) { $expander = $prior; $items = $expander.nextUntil(':not(details)'); }
        } else { var $list = $(oItem).closest('.accordion-list'); $expander = $list.prev(); $items = $list.find('.accordion-item'); }
        if ($expander && $expander.length > 0) {
            var itemsOpen = 0; var itemsClosed = 0; $items.each(function () { if ($(this).prop("open") == true) { itemsOpen = itemsOpen + 1; } else { itemsClosed = itemsClosed + 1; } }); var state = ''; var buttonClass = ''; if (itemsOpen >= itemsClosed) { state = collapseTag; buttonClass = 'shrink'; } else { state = expandTag; buttonClass = 'grow'; }
            var button = $expander.find('button'); $(button).html(state); $(button).removeClass('grow shrink'); $(button).addClass(buttonClass);
        }
    }
    $(".accordion-item .accordion-trigger").on("click", function () { if ($(this).data("canfire") !== 'false') { accordionFire(this); $(this).find(".accordion-item-button").addClass('hidefocus'); } }); $(".accordion-item .accordion-trigger").on("blur", function () { $(this).find(".accordion-item-button").removeClass('hidefocus'); }); $(".SubHubNavigationPageFeature .accordion-item .accordion-trigger").on("click", function () {
        var block = $(this).closest('.block-left')
        block.toggleClass('open');
    }); $(".accordion-item").each(function (index) { accordionItemToggle(this, true); }); $(".accordion-toggle").show(); $('.process-nav li:first-child').addClass('selected'); $(".accordion-item-button.flags-contextual-images-button").on("click", function (e) { var toggleLabel = $('.accordion-item-button.flags-contextual-images-button .accordion-title'); if (toggleLabel.hasClass('open')) { toggleLabel.removeClass('open'); toggleLabel.html('See more designs in context'); } else { toggleLabel.addClass('open'); toggleLabel.html('Hide Contextual images'); } }); $('.accordion-expand').click(function (e) {
        var trigger = $(this); var target = $(e.target).data('target'); var scope; var items; if (target) { scope = $('#' + target); items = $(scope).find('.accordion-item'); } else { scope = trigger.closest('.accordion-toggle'); items = scope.nextUntil(':not(details)'); }
        if ($(scope).find('.accordion-expand').hasClass('grow')) {
            $(items).each(function () {
                var itemContent = $(this).find(".accordion-content"); $(this).prop("open", true); $(this).find(".accordion-close").show(); $(this).find(".accordion-open").hide(); $(this).find("summary").css("color", "#121212"); $(".book-mobile-menu").addClass("accordion-heading-open"); $(this).find(".accordion-item-button").attr("aria-expanded", "true"); var details = $(this).closest('details'); if (!details.hasClass('accordion-heading-open')) { $(itemContent).animate({ height: "toggle" }, animationDuration, function () { $(this).show(); }); }
                $(this).addClass("accordion-heading-open");
            }); $(trigger).removeClass('grow'); $(trigger).addClass('shrink'); $(trigger).html(collapseTag); calculateExpanderState(this);
        } else { $(items).each(function () { var itemContent = $(this).find(".accordion-content"); $(this).find(".accordion-close").hide(); $(this).find(".accordion-open").show(); $(this).find("summary").css("color", "#336699"); $(this).removeClass("accordion-heading-open"); $(".book-mobile-menu").removeClass("accordion-heading-open"); $(this).find(".accordion-item-button").attr("aria-expanded", "false"); $(itemContent).animate({ height: "toggle" }, animationDuration, function () { $(this).closest('details').removeProp("open"); $(itemContent).hide(); }); }); $(scope).find('.accordion-expand').removeClass('shrink'); $(scope).find('.accordion-expand').addClass('grow'); $(trigger).html(expandTag); calculateExpanderState(this); }
    }); $('.accordion-item.inline-accordion').each(function () { var $next = $(this).next(); if (!$next.hasClass('accordion-item')) { $(this).addClass('last-in-seq'); } }); setTimeout(function () {
        $('.accordion-toggle.inline-accordion').each(function () {
            var items = $(this).nextUntil(':not(details)'); var ids = ''; items.each(function (i, el) { ids += $(el).attr('id') + ' '; })
            $('button', this).attr('aria-controls', ids);
        });
    }, 100);
})(jQuery);;

/****** FILE: themes/dia-theme/javascript/forms.js *****/

(function ($) {
    "use strict"; var
    arrErrors = [], arrForms = {}; var currentStep = null; var requiredFields = {}; function isValidEmailAddress(emailAddress) { var pattern = new RegExp(/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i); return pattern.test(emailAddress); }
    function validateErrorAdd(sID, oLabel, sError) {
        var
        sAtom = validateLabelID(oLabel, sID), oSpan = oLabel.find('span.form-error'), sIcon = ' <br /><span class="sr-only">Error: </span><i class="fa fa-warning" aria-hidden="true"></i> ' + sError; arrErrors.push('<span class="form-error-item" data-field="' + sID + '" data-label="' + sAtom + '" role="link" tabindex="-1">' + sError + '</span>'); if (oSpan.length === 0)
            oSpan = $('<span/>', { html: sIcon, "class": "form-error" }).appendTo(oLabel)
        else
            oSpan.html(sIcon);
    }
    ...
    function validateFieldEmail(sID) {
        var
        sValue = $('#' + sID).val().trim(), bFail = (sValue === '' || !isValidEmailAddress(sValue)), oLabel = $("label[for='" + sID + "']"); if (bFail) {
            var
            sAtom = validateLabelText(oLabel.html()), sError = 'Enter a valid email address in the <em>' + sAtom + '&nbsp;</em> field'; validateErrorAdd(sID, oLabel, sError);
        }
    }
    function validateFieldFile(sID) {
        var
        bFail = $('#' + sID).val().trim() === '', oLabel = $("label[for='" + sID + "']"); if (bFail) {
            var
            sAtom = validateLabelText(oLabel.html()), sError = 'Select a file for <em>' + sAtom + '&nbsp;</em>'; validateErrorAdd(sID, oLabel, sError);
        }
    }
    function validateFieldGeneric(sID) {
        ...
        bFail = $('#' + sID).val().trim() === '', oLabel = $("label[for='" + sID + "']"); if (bFail) {
            var
            sAtom = validateLabelText(oLabel.html()), sError = 'The <em>' + sAtom + '&nbsp;</em> field cannot be empty'; validateErrorAdd(sID, oLabel, sError);
        }
    }
    function validateFieldSelect(sID) {
        var
        bFail = $('#' + sID).val() === '', oLabel = $("label[for='" + sID + "']"); if (bFail) {
            var
            sAtom = validateLabelText(oLabel.html()), sError = 'Select an option for the <em>' + sAtom + '&nbsp;</em> field'; validateErrorAdd(sID, oLabel, sError);
        }
    }
    function validateForm(sForm) {
        if (currentStep != null) {
            arrErrors = []; $(".form-error").html(""); $.each(arrForms[sForm], function (key, value) {
                if (requiredFields[key] != null) { doValidation(key, value); }
                if (arrErrors.length > 0) { $('form').attr('data-validate', 'stop'); } else { $('form').attr('data-validate', ''); $('.messages-block.bad-msg').remove(); $('.error-container').remove(); }
            }); validateFormShowErrors(sForm); return (arrErrors.length === 0);
        } else { arrErrors = []; $(".form-error").html(""); $.each(arrForms[sForm], function (key, value) { doValidation(key, value); }); validateFormShowErrors(sForm); return (arrErrors.length === 0); }
    }
    function doValidation(key, value) { switch (value) { case 'checkbox': validateFieldCheckbox(key); break; case 'email': validateFieldEmail(key); break; case 'file': validateFieldFile(key); break; case 'select': validateFieldSelect(key); break; default: validateFieldGeneric(key); } }
    function validateFormShowErrors(sForm) {
        var
        oCallout = $('#' + sForm).find(".messages-block.bad-msg.callout-alert").first(); if (arrErrors.length > 0) {
            var
            sPlural = (arrErrors.length === 1) ? '' : 's', sMsg = '<h2 tabindex="-1" class="imitate-h4">Correct the following error' + sPlural + ' and try again:</h2><ul>'; jQuery.each(arrErrors, function (index, item) { sMsg += '<li>' + item + '</li>'; }); sMsg += '</ul>'; if (oCallout.length) { oCallout.html(sMsg); }
            else
                oCallout = $('<div/>', { html: sMsg, "class": "messages-block bad-msg callout-alert" }).prependTo('#' + sForm); var offset = oCallout.offset(); $('html, body').animate({ scrollTop: offset.top, scrollLeft: offset.left }); oCallout.find('h2').focus();
        }
    }
    function validateInit() {
        $("form").each(function (iIndex) {
            if ($(this).attr("id") !== 'search-form' && $(this).attr("id") !== 'Form_InlineFeedbackForm' && $(this).attr("id") !== 'GNZUserForm_Form') {
                var sForm = $(this).attr("id"); $(this).noValidate = true; arrForms[sForm] = {}; $(this).find(":input").each(function () { if ($(this).attr('required') === "required") { $(this).removeAttr('required'); arrForms[sForm][$(this).attr("id")] = $(this).attr('type') || $(this).prop("tagName").toLowerCase(); } }); $(this).find(":submit").each(function (iIndex) {
                    $(this).attr('formnovalidate', 'true'); $(this).on("click", function (event) {
                        var form = $(this).closest('form'); var currentStepNo = $(form).find('.step-buttons li.current .step-button-jump').html()
                        var currentStepNoAdjusted = currentStepNo - 1; if (currentStepNoAdjusted >= 0) {
                            var formStep = $(form).find('fieldset .form-step'); currentStep = formStep[currentStepNoAdjusted]; var fieldsOnCurrentStep = $(currentStep).find('.field'); requiredFields = {}; var i; for (i = 0; i < fieldsOnCurrentStep.length; i++) {
                                var key = $(fieldsOnCurrentStep[i]).attr('id'); key = 'GOESUserForm_Form_' + key; var type = $(fieldsOnCurrentStep[i]).attr('class'); if (type.indexOf("dropdown") >= 0) { type = 'select'; } else { type = 'text'; }
    ...
    function validateLabelID(oLabel, sField) {
        var
        sID = oLabel.attr("id"); if (typeof sID === "undefined") { sID = sField + '_' + 'UrlAtomLabel'; oLabel.attr("id", sID); }
        return sID;
    }
    function validateLabelText(sText) {
        var
        arrText = sText.split('('), arrAtom = arrText[0].split('<'); return arrAtom[0].trim();
    }
    function validateScrollToError(oLink) {
        var
        sField = $(oLink).data("field"), sLabel = $(oLink).data("label"), offset = $("#" + sLabel).offset(); $('html, body').animate({ scrollTop: offset.top, scrollLeft: offset.left }); $("#" + sField).focus();
    }
    function findCounter($textarea) {
        $counter = $('.word-count[data-textarea=' + $textarea.attr('id') + ']'); if ($counter.length > 0) { return $counter; }
        var $textareaContainer = $textarea.closest('.field'); var $counter = $textareaContainer.next(); if (!$counter.hasClass('word-count')) { return null; }
        return $counter;
    }
    var truncateWords = function ($textarea, limit) {
        if (limit == 0) { return; }
        var s = $textarea.val(); if (s.length == 0) { return; }
        var regex = /\s+/gi; var words = s.trim().replace(regex, ' ').split(' '); if (words.length <= limit) { return; }
        var nToRemove = words.length - limit; s = s.trim(); while (nToRemove > 0) { s = s.replace(/(\s+)(\S+)$/, ''); nToRemove--; }
        $textarea.val(s);
    }
    var counterImpl = function (event, enforceLimits) {
        var $event = $(event.target); if (typeof enforceLimits === 'undefined') { enforceLimits = false; }
        var $textarea = $event.closest('textarea'); var limit = parseInt($textarea.attr('data-max-words'), 10); if (limit === 0) { return; }
        if (!limit) { limit = 200; }
        var hardLimit = $textarea.attr('data-hard-limit') == 'true'; var $counter = findCounter($textarea); if ($counter == null || $counter.length == 0) { return; }
        if (hardLimit && enforceLimits) { truncateWords($textarea, limit); }
        var value = $textarea.val(); var wordCount; if (value.length == 0) { wordCount = 0; } else { var regex = /\s+/gi; wordCount = value.trim().replace(regex, ' ').split(' ').length; }
        var color = 'black'; if (wordCount > limit) { color = 'red'; }
        var w = 'word'; if (wordCount != 1) { w = 'words'; }
        var limitText = ' (' + limit + ' max'
        if (hardLimit) { limitText += ', any extra words will be truncated'; }
        limitText += ')'; var html = '<span style="color:' + color + '">' + wordCount + '</span> ' + w + limitText; $counter.html(html);
    }; var pending = {}; var counter = function (event, enforceLimits) {
        if (!enforceLimits && pending[event.target.id]) { return; }
        var delay = 250; if (enforceLimits) { delay = 0; }
        pending[event.target.id] = true; setTimeout(function () { counterImpl(event, enforceLimits); pending[event.target.id] = false; }, delay);
    }; var onTextareaBlur = function (event) { counter(event, true); }
    $(document).ready(function () { validateInit(); $(document).on("click", ".form-error-item", function () { validateScrollToError(this); }); $(document).on("keypress", ".form-error-item", function () { validateScrollToError(this); }); $('#count').click(counter); $('textarea').change(counter); $('textarea').keydown(counter); $('textarea').keypress(counter); $('textarea').keyup(counter); $('textarea').blur(onTextareaBlur); $('textarea').focus(counter); $('textarea').each(function (i, el) { counter({ target: el }); }); });
})(jQuery);;

/****** FILE: themes/dia-theme/javascript/focus.js *****/

(function ($) {
    $(document).ready(function () {
        $("a, button, .btn, details summary, input").click(function () { $("a, button, .btn, details summary, input").removeClass("selected"); $(".info-section a .inner").removeClass("selected"); $(this).addClass("selected"); }); $(".info-section a").click(function () { $("a, button, .btn, details summary, input").removeClass("selected"); $(".info-section a .inner").removeClass("selected"); $(this).find(".inner").addClass("selected"); }); var sPageURL = window.location.search.substring(1); var sURLVariables = sPageURL.split('&'); for (var i = 0; i < sURLVariables.length; i++) {
            var sParameterName = sURLVariables[i].split('='); if (sParameterName[0] == 'comments')
            { if (sParameterName[1] == 'true') { setTimeout(function () { $('body').animate({ scrollTop: $('#comments').offset().top }, 500); }, 200); } }
        }
    });
})(jQuery);;

/****** FILE: themes/dia-theme/javascript/printpage.js *****/

$(document).ready(function () { $(document).on("click", "#print-page-button", function (e) { window.print(); e.preventDefault(); }); });;
